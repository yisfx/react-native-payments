...
export default class PaymentRequestUpdateEvent {
    ...
    return (
      NativePayments.handleDetailsUpdate(normalizedDetails, DOMException)
        ...
        .then(() => {
          ...
          if(target.isFirstOrderReview==undefined || target.isFirstOrderReview){
            if (target._details.shippingOptions && target._details.shippingOptions.length > 0) {
              let selected:any;
              target._details.shippingOptions.map(op=>{
                if(op.selected){
                  selected=op
                }
              })
              if(!selected){
                selected=target._details.shippingOptions[0].id
              }
  ...
  updateWith(
    ...
    if (target._updating === true) {
      return;// throw new Error('InvalidStateError');
    }
