...
export default class PaymentRequest {
  ...
  _setupEventListeners() {
    ...
    if (IS_IOS) {
      ...
      if(!!this._shippingOptionChangeSubscription){
      ...
      if(!!this._shippingAddressChangeSubscription){
        DeviceEventEmitter.removeSubscription(
  ...
  _handleUserAccept(details: {
    ...
    if(!!this._acceptPromiseResolver)
      this._acceptPromiseResolver(paymentResponse);
  }
  ...
  show(placeorder:Function): Promise<PaymentResponseType> {
    this._acceptPromise = new Promise((resolve, reject) => {
      if(!placeorder)
        this._acceptPromiseResolver = resolve;
      else
        this._acceptPromiseResolver = placeorder;
